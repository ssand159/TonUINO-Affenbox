...
#define ROTARY_ENCODER_PIN_A 5 //Default 5; Pocket A0
#define ROTARY_ENCODER_PIN_B 6 //Default 6; Pocket A1
#define ROTARY_ENCODER_PIN_SUPPLY 8 //comment if you dont want to use an IO pin 
#endif
...
static void nextTrack(uint16_t track) {
  ...
  if (myFolder->mode == 2 || myFolder->mode == 8) {
    if (currentTrack != numTracksInFolder) {
      ...
      mp3.playFolderTrack(myFolder->folder, currentTrack);
#ifdef DEBUG
      Serial.print(F("Album aktiv > next Track "));
...
#ifdef DEBUG
      Serial.println(F("Queue End > repeat"));
#endif
      currentTrack = 1;
...
static void previousTrack() {
  ...
  if (myFolder->mode == 3 || myFolder->mode == 9) {
    ...
    {
#ifdef DEBUG
      Serial.print(F("Anfang Queue > end"));
#endif
...
void playFolder() {
  ...
  while (!isPlaying() && counter <= 50) {
    ...
    if (counter >= 50) {
#ifdef DEBUG
      Serial.println(F("playFolder > Track not starting.")) ;
#endif
    }
  }
}
