...
class Modifier {
  public:
    ...
    virtual bool handleShortCutButtons() {
      return false;
    }
...
class Locked: public Modifier {
  public:
    ...
    virtual bool handleRFID(nfcTagObject *newCard) {
      Serial.println(F("== Locked::handleShortCutButtons() -> LOCKED!"));
      return true;
    }
    Locked(void) {
...
#ifdef SHORTCUTBUTTONS
#define buttonShortCutOnePin 5
#define buttonShortCutTwoPin 6
#define buttonShortCutThreePin 7
#endif
...
#ifdef SHORTCUTBUTTONS
Button shortCutButtonOne(buttonShortCutOnePin);
Button shortCutButtonTwo(buttonShortCutTwoPin);
Button shortCutButtonThree(buttonShortCutThreePin);
...
#endif
#ifdef SHORTCUTBUTTONS
  shortCutButtonOne.read();
  shortCutButtonTwo.read();
  shortCutButtonThree.read();
#endif
}
...
#ifdef SHORTCUTBUTTONS
if (shortCutButtonOne.wasReleased()) {
  if (activeModifier != NULL)
        if (activeModifier->handleShortCutButtons() == true)
          return;
...
if (shortCutButtonTwo.wasReleased()) {
  if (activeModifier != NULL)
        if (activeModifier->handleShortCutButtons() == true)
          return;
...
if (shortCutButtonThree.wasReleased()) {
  if (activeModifier != NULL)
        if (activeModifier->handleShortCutButtons() == true)
          return;
...
void adminMenu(bool fromCard = false) {
  ...
  if (subMenu == 1) {
    ...
    #ifdef SHORTCUTBUTTONS
    uint8_t shortcut = voiceMenu(7, 940, 940);
    #endif
    #ifndef SHORTCUTBUTTONS
